...
Continuation marks originated in PLT Scheme (now Racket \cite{plt-tr1}). However, in spite of their usefulness, continuation marks have remained absent from programming languages at large. One reason for this may be that retrofitting virtual machines to accommodate the level of stack inspection continuation marks must provide is expensive, especially when the virtual machine uses the host stack for efficiency.

JavaScript, for example, enjoys ubiquity as the \emph{lingua franca} of the web and is an ideal candidate for continuation marks. One approach to adding continuation marks to JavaScript would be to alter numerous mature implemenations--indeed, Clements et al. accomplished this for Mozilla's Rhino compiler \cite{clements2008implementing}--and deploy them all. This leaves the devices that will never upgrade in the dust. The effort required to add continuation marks with this approach is immense. Nevertheless, it could benefit from such an infusion. Another approach would be to express continuation marks in terms of a subset of JavaScript--the smaller, the better. This macro-style approach would keep in the spirit of JavaScript as the \emph{lingua franca} of the web by desugaring our enhanced language into JavaScript.

but we actually did it for the lambda calculus

With such a transformation, higher-order language semanticists would not need to reconcile the feature with other features in the language (provided they had already done so with $\lambda$) and their compiler writers would not need to worry about complicating their implementations (for the same reason).
