...
Now consider the reduction of the same program transformed. We apply the transformation just-in-time as we reduce to reduce term size and promote clarity.
\begin{schemedisplay}
...
We do this by overloading $\mathcal{C}_{\mathrm{cps}}$ to accommodate contexts which allows us to formally relate $E[e]$ and $\Ccps{E[e]}$. We first define
\begin{plaindef}
\[
\xi(E)=\begin{cases}
...
\]
\end{plaindef}
to denote the \scheme'flags' argument and assume that the \scheme'marks' argument is \scheme|Cpcps[chiE]|. We can now define $\mathcal{C}_{\mathrm{cps}}$ over contexts:
...
This allows us to define $\mathcal{C}_{\mathrm{cps}}$ over a context-expression pair with mildly unorthodox notation.
