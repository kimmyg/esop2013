...
The definition of $\mathcal{C}$ entails a defined transformation of each syntactic form of \cm.
...
For this reason, we adjust $\mathcal{C}'$ so that
\begin{equation}
\Cp{p}=\app{\app{\C{p}}{\false}}{\C{\nil}}
\end{equation}
and recognize that a fully-realized transform must be made aware of this.
...
(lambda (kont)
  (lambda (flag)
    (lambda (marks)
      (((C[mark-expr]
         (lambda (mark-value) ((lambda (k_1) 
                                 (k_1 (((marks (lambda (x) x)) (lambda (z) z)) (lambda (z) z))))
                               (lambda (t) 
                                 ((lambda (k_2) (k_2 ((f C[(SND t)]) t)))
                                   (lambda (rest) 
