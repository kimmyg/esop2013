...
We term our transformation from \cm\ to \lv\ $\mathcal{C}$, as in \emph{compile}, since we are, in essence, compiling away continuation marks. The transformation $\mathcal{C}$ is defined over each syntactic form of \cm. In order to preserve the meaning of \cm, $\mathcal{C}$ must commute with evaluation. More precisely, the following property should hold for programs $p\in\lambda_{cm}$:
\[
\begin{array}{ccc}
p & \cmrrs & v\\
...
The essence of \cm\ is that programs can apply information to and observe information about the context in which they are evaluated. Programs in \lv\ have no such facility. We can simulate this facility by explicitly passing contextual information to each term as it is evaluated. We can define $\mathcal{C}$ to transform \scheme'wcm' directives to manipulate this information and \scheme'ccm' directives to access it. In other words, we can transform \cm\ programs to mark-passing style.

However, marks alone do not account for the tail-call behavior specified by rule \ref{wcm-collapse}. Since tail-call behavior is observable (if indirectly) to \cm\ programs, we must also provide to each term information about the position in which it is evaluated. Specifically, each \scheme'wcm' directive must be 

can frame this information in terms of the evaluation context and 


This realization guides the intuition behind the transform: to propagate contextual information to each expression, manipulating it appropriately as the context changes.
